/**
 * @author Sanjay
 * Min-Max Algorithm for mid-end phase 
 * Player = Black
 */
...
public class MiniMaxGameBlack {
	...
	public String miniMaxAlg(char[] board, int depth,char player,char opponent){
		...
		ArrayList<char[]> moves = moveGenerator('m',player,opponent);
		for(char[] move : moves){
	...
	public int minMax(int depth, char player, char opponent){
		...
		ArrayList<char[]> moves = moveGenerator('m',opponent,player);
		if(moves.isEmpty()){
			evaluatedNodes++;
			return estimate('m',board);
		}	
	...
	public int estimate(char phase, char[] b){
		...
		if(phase == 'o' || phase == 'O')
			staticEstimate = numOfBlack - numOfWhite;
		else{
			if(numOfWhite <= 2) staticEstimate = 10000;
			else if(numOfBlack <= 2) staticEstimate = -10000;
			else {
				numWhiteMoves = findWhiteMoveCount(b);
				if(numWhiteMoves == 0)  staticEstimate = 10000;
				else staticEstimate = (1000*(numOfBlack - numOfWhite)-numWhiteMoves);
			}
		}
		return staticEstimate;
	}
	
	public int findWhiteMoveCount(char[] b){
		int count = 0;
